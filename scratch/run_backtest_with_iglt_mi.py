...
# European Pool with IGLT.MI instead of IGLT.L
european_pool_mi = [
...
print("Downloading historical data with IGLT.MI replacing IGLT.L...")
df_raw = yf.download(european_pool_mi, start=start_date, end=end_date, progress=False)
...
# Clean prices
prices_clean = df[european_pool_mi].ffill().bfill()
print(f"Data range: {prices_clean.index[0].strftime('%Y-%m-%d')} to {prices_clean.index[-1].strftime('%Y-%m-%d')}")
print("Number of non-NaN values for IGLT.MI:", df['IGLT.MI'].notna().sum())
...
print(f"HRP Cum Final Value (After-Tax): {hrp_cum.iloc[-1] * 100000.0:.2f}")
print(f"S&P 500 Cum Final Value: {sp500_cum.iloc[-1] * 100000.0:.2f}")
print(f"60/40 Cum Final Value (After-Tax): {sixty_forty_cum.iloc[-1] * 100000.0:.2f}")
