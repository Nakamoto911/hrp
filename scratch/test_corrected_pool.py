...
# Corrected European Ticker List
corrected_tickers = [
    'SXR8.DE',  # S&P 500
    'SXRZ.DE',  # Nikkei 225 (corrected from EXX5.DE)
    'SXRW.DE',  # FTSE 100 (corrected from EXS1.DE)
    'SXRT.DE',  # EuroSTOXX 50
    'IS3N.DE',  # MSCI EM
    'IUSM.DE',  # US 10Y Treasuries
    'IS0L.DE',  # Bund Allemand
    'XJSE.DE',  # JGB Japonais
    'IGLT.MI',  # U.K. Gilts (corrected from IGLT.L)
    'IBCQ.DE',  # Credit IG
    'IHYG.MI',  # Credit HY
    '4GLD.DE',  # Gold
    'CRUD.MI',  # Oil
    'COPA.MI',  # Copper
    'AIGP.MI',  # Agriculture (Precious Metals)
    'IS3S.DE',  # Equity Value (corrected from 5MVW.DE)
    'IS3R.DE',  # Equity Momentum (corrected from 36BZ.DE)
    'IS3Q.DE',  # Equity Quality (corrected from QDVB.DE)
    'IQQ0.DE',  # Equity Defensive (corrected from IUS3.DE)
    'BTCE.DE'   # Bitcoin
]
...
for t in corrected_tickers:
    ...
    if not df_t.empty:
        ...
        # Convert Series values to flat 1D array
...
print("\n--- RESULTS WITH CORRECTED TICKERS & CLEANED PIPELINE ---")
