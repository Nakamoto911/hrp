...
def recursive_bisection(cov: pd.DataFrame, sort_items: List[str]) -> pd.Series:
    """
    Allocates portfolio weights recursively using inverse-variance cluster bisection.
    """
...
def optimize_hrp(cov: pd.DataFrame, linkage_method: str = 'single') -> pd.Series:
    """
    Main HRP optimization entry point.
    Computes the distance matrix, performs hierarchical clustering, 
    reorders the covariance matrix, and allocates weights.
    """
    ...
    # Compute Euclidean distance between columns of the distance matrix
    # dist_cols[i, j] = sqrt(sum_k (dist[i, k] - dist[j, k])^2)
    ...
    # linkage methods: 'single', 'complete', 'ward'
    ...
    w = recursive_bisection(cov, sort_items)
